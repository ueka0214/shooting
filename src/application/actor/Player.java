...
public class Player extends Actor {
	...
	static final double SPEED = 1.25;// キャラクターの速さ
	...
	public Player(IWorld world, Input input, Vector2 position) {
		super(world, position, WIDTH, HEIGHT, ImageManager.getImage(ActorID.ACTOR_PLAYER), 5);
		this.input = input;
	...
	private void shot() {
		shotTimer--;
		if (input.isSpace() && shotTimer < 0) {
			Vector2 pos = new Vector2(this.position.x + WIDTH * 3 / 7, this.position.y);
			world.add(ActorID.ACTOR_PLAYER_BULLET, new Bullet(world, pos));
